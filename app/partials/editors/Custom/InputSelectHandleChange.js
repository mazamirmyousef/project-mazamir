...
const InputSelectHandleChange = (props) => {
  ...
  console.log("valuevalue" , namePath)
            ...
            <Controller
                as={
                    <Select label={label} size="small" error={hasError}>
                        ...
                        {data.map(item =>
                            <MenuItem value={item.id}  onClick={() => handleChange(item.id)} key={item.id}>{item.desc}</MenuItem>
                        )}
                    </Select>
                }
                name={name}
...
export default InputSelectHandleChange;